...
categorical_columns.json, freq_maps.json, group_stats.json,
review_policy.json) and reuses `fraud_radar.features.build_features` so a
raw transaction record is turned into the same feature frame the model was
trained on -- including the card1_count/addr1_count/P_emaildomain_count/
uid_count and card1_mean_amt/card1_addr1_count/card1_addr1_mean_amt/
card1_P_emaildomain_count/card1_P_emaildomain_mean_amt columns, computed
from the saved training-slice maps rather than from the single row being
scored (which would otherwise put every count at 1.0 and every mean at the
row's own amount). `time_since_prev_card1`/`time_since_prev_card1_addr1`
are the one exception -- a single-row frame has no history, so they always
come back NaN at serving time; a known limitation, not a bug (see README).
...
class FraudScorer:
    ...
    def load(cls, review_threshold: float | None = None) -> FraudScorer:
        ...
        return cls(booster, calibrator, feature_cols, cat_cols, freq_maps, group_stats_maps, review_threshold)
    ...
    def _to_feature_row(self, transaction: dict) -> pd.DataFrame:
        ...
        feat = build_features(row, freq_maps=self.freq_maps, group_stats_maps=self.group_stats_maps)
